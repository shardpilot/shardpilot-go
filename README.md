...
`ExperimentsEnabled: true` opts this client into the experiment-assignment consumer (ADR-0259). **The default is off — dark**: zero experiment code paths execute (no subject-id mint, no fetch, no revalidation goroutine, no fact emission, no new persistence keys), and the surface refuses `ErrExperimentsNotConfigured`. The flag requires `RemoteConfigURL` (the assignment endpoint lives on the same control-plane host, path-swapped to `/api/v1/runtime/experiments/assignment`) and authenticates with the same publishable `APIKey`.

- **Dark phase, stated plainly:** the platform's experimentation flags are OFF everywhere today, so an enabled consumer receives `403` on every fetch until platform enablement — the expected fail-closed posture. Until the control-plane auth leg grants publishable keys the `experiment_assignment_read` scope, only an explicitly scoped runtime token authenticates (`401` otherwise). The exposure/outcome lane is dark **end-to-end**: the analytics service rejects `experiment_exposure`/`experiment_outcome` from publishable client keys until the platform's producer-lane decision lands — an emitted fact comes back as a per-event reject, and this SDK deliberately relies on that server-side block staying authoritative.
