...
- Three import paths: the root `shardpilot` package (analytics), `pkg/crash` (crash reporting), and `pkg/consentpolicy` (fail-closed plan validation).
...
- **Consent-regime plans (`pkg/consentpolicy`) — what this release's plan-text validation covers, and what it does not.** `Prepare` verifies a plan for one scoped player operation and, **in this release, uses no plan at all**: there is no key to authenticate one with, an unauthenticated plan is not evidence, so every verdict is closed on every axis (`PlanUsed()` false, the optional-analytics choice defaulted OFF with an explicit grant required, no crash profile offered, server analytics denied, child rules applied and every operation blocked — note that STRICT means **ask, with the choice pre-set off**, never "do not ask"). Validation still runs first, so a malformed or out-of-scope handoff is named precisely, and it covers **raw UTF-8 validity of the plan bytes, exact-case unique keys at every depth, presence of every required key at every depth (absence is never a default), `null` for any key but `signature` (present-and-null is not absent, and `encoding/json` cannot tell them apart), the closed refusal vocabulary, and EOF after the plan**. Two things it deliberately does **not** cover, because `encoding/json` and `time.Parse` repair rather than refuse them: **JSON escape-level unpaired surrogates**, which decode to U+FFFD, and **out-of-range RFC 3339 zone offsets**, which normalise. Neither can change a decision while no plan is used; both are **required of the release-2 verifier**, the one that has a verification key, and are recorded here so that release does not have to rediscover them.
