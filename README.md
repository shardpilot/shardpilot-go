...
| `RemoteConfigCachePath` | no | File for the durable last-known-good remote-config cache record. Empty keeps the cache in-memory only. Independent of `SpoolDir`; never enables consent persistence. |
| `SpoolDir` | no | Opt-in state directory for the bounded disk spool and the persisted consent decision (`spool.json`, `consent.json`, `spool-wipe-owed`; dir `0700`, files `0600`). Empty (default) keeps the queue memory-only — nothing is ever written to disk. |
...
| `OnSpoolDeadLetter` | no | `func(SpoolDeadLetter)` called with every event the spool drops undelivered (capacity, 7-day retry-age expiry, terminal outcome, consent purge/refusal). Panic-recovered like `OnBatchResult`. |
...
- **Explicit-fetch-only.** The SDK never refreshes on its own — no polling, no `Cache-Control` interpretation, no experiment assignment or client-side rule evaluation. Every fetch is one `GET`, revalidated with the cached ETag.
