...
| `ExperimentAssignmentCachePath` | no | File for the durable per-experiment last-known-good assignment records (same file discipline and bounded read-back as `RemoteConfigCachePath`; one client per cache path). Empty keeps assignment records in-memory only. |
