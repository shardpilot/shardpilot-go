...
- `IngestURL` (and, for crash reporting, the `pkg/crash` base URL) is the base URL of the ShardPilot ingest deployment you were given, or of a local stack you run yourself. The SDK does not discover endpoints.

## What it does

- Builds and sends app-first event envelopes to `POST {IngestURL}/v1/events:batch` with bearer-token auth.
- Synchronous `Track`, bounded async `Enqueue`, `Flush`, and `Close`; in-memory delivery stats via `Snapshot` (including a per-status breakdown in `Stats.ByStatus`), a bounded rejection history via `Rejections()`, and an optional `OnBatchResult` callback that surfaces the server's per-event outcomes (which events were rejected, suppressed, observed, or folded as duplicates).
- Bounded batching (default 25 events, capped at 100) with retry of retryable HTTP responses; a `429`'s `Retry-After` hint defers the next automatic publish attempt, and retryable failures without a hint (server unreachable, `5xx` without the header) fall back to client-side exponential backoff with full jitter on their OWN clock, independent of `FlushInterval` — every failure, the first included *(the FIRST failure's independent delay is unreleased — at `v0.6.1-alpha` `backoffCeiling(1)` is 0, so the first retry waits for the flush tick instead)*, waits a random duration in [1s, ceiling], the ceiling doubling from the third consecutive failure up to 60s, reset on success (explicit `Flush`/`Close` still try immediately). Memory-only queue (no durable on-disk queue). Event ids and timestamps are stamped once at intake, so a re-sent batch is de-duplicated by the service instead of double-counted.
...
`v0.6.1-alpha` is the latest tag. It is `v0.6.0-alpha` with two internal agent skills deleted and nothing else — no Go source differs between them.

**This README is written against `main`, which is ahead of that tag.** The behaviours listed here are NOT in `v0.6.1-alpha` and are marked *(unreleased)* where they appear: `Config.DisableRequestCompression` **and the entire request-compression behaviour it opts out of** — the tag has no gzip path at all, so every body there is sent uncompressed; the 15-second `FlushInterval` default, which is 1 second at the tag; the goroutine-label
fix, without which a `runtime/pprof` label containing `]` can reach `Thread.Name`
in a crash payload on Go 1.27+; and the independent pacing of the FIRST retry — at the tag `backoffCeiling(1)` returns 0, so a first failure without a `Retry-After` hint waits for the next flush tick rather than its own jittered delay. The rest of the retry-clock work is unreleased with it: the tag has no `nextPacingWake`, so a consent-retry deadline that elapsed while the worker was busy with another select case, a deadline armed by a concurrent synchronous `Track` after the worker had parked, and spool entries reloaded at startup all wait for the flush tick there rather than their own clock. Everything else describes the pinned release. `v0.6.0-alpha` shipped the optional pseudonymous actor key on crash reports — `Event.AnonymousID`, the ONLY field the service keys the actor on, with `Event.SessionID` alongside it as session linkage that is never used as the actor key (both `omitempty`, so a client that does not opt in keeps a byte-identical wire shape) — the dark opt-in experiment-assignment consumer with consent-gated exposure/outcome producers, the dark opt-in remote-config targeting-attribute pass-through, the phase-D crash capture opt-ins (self-module debug-id fill and all-goroutine threads), and the opt-in client-side consent floor (`Config.ConsentFloor`), on top of the v0.5.0-alpha remote config client, disk spool, schema-revision declaration and full-jitter retry backoff documented in this README. **Do not pin an earlier tag.** Two tags — `v0.5.0-alpha` and `v0.6.0-alpha` — distribute eight internal agent-skill files through `go get`; `v0.6.1-alpha` is the deletion-only patch that removes them. Tags at `v0.4.0-alpha` and below predate those files and do not carry them, but they also predate most of what this README documents. If you need a release without the features listed above, wait for one cut from the cleaned tree rather than reaching back.

For analytics only, `v0.1.2` is available. **`v0.1.0` is retracted** in the module's `go.mod`; use `v0.1.2` or `v0.2.0-alpha`. Not "or later" — that phrasing pointed at `v0.5.0-alpha` and `v0.6.0-alpha`, the two tags this change exists to stop distributing, and contradicted the prohibition four lines above. The pin at the top of this file is the current release. `v0.1.2` requires **Go 1.24+** — it is the Go 1.24 modernization release and its `go` directive is immutable. The **1.25** baseline stated above applies to the next release and to the current `main`, not retroactively to any existing tag.
...
| `FlushInterval` | no | *(the 15s default is unreleased — `v0.6.1-alpha` uses 1s)* Default 15s (was 1s). How long a **partial** batch waits before publishing — not a heartbeat: an empty batch publishes nothing, so a process that tracks nothing makes no requests at either value. At 1s a service emitting an event every few seconds never filled a batch, so every event became its own request; 15s lets batching actually happen. A full `BatchSize` still publishes immediately and `Flush()` on demand. Retry pacing does not follow this value. Set it explicitly for a different cadence; an explicit value below the default is honoured. |
...
| `ExperimentsEnabled` | no | Default `false` — dark: zero experiment code paths. `true` opts into the experiment-assignment consumer (requires `RemoteConfigURL`; same publishable `APIKey` auth; see "Experiments"). |
...
| `RemoteConfigAttributesEnabled` | no | Default `false` — dark: the remote-config fetch URL is byte-identical to today's attribute-less path. `true` opts into the targeting-attribute pass-through: attributes set via `SetRemoteConfigAttributes` ride the config GET as query parameters so server-side delivery rules can target this client — but **only while consent is granted** (unknown/denied fetch attribute-less; see "Remote config"). Requires `RemoteConfigURL`. |
...
| `RejectionCapacity` | no | Unreleased: rejection-history entry cap; default 64 for non-positive values, oldest evicted first. See "Batch verdicts". |
...
*(This whole paragraph is unreleased — `v0.6.1-alpha` has no gzip path at all: zero
`gzip` references in `client.go` at that tag, against two on `main`. At the pin
every body is sent uncompressed.)* Request bodies over 1 KiB are sent
**gzip-compressed** with `Content-Encoding: gzip` (batch publishes and consent writes alike). The ingest cap (`INGEST_MAX_BODY_BYTES`, default 8 MiB) applies to the **uncompressed** body, so compression buys throughput and not headroom — `BatchSize` still governs how large a body gets. A deployment that cannot read the coding answers `400` with detail code `unsupported_content_encoding` (or `invalid_content_encoding` if the stream arrived unreadable); the client latches compression off for the process and re-sends the same batch uncompressed, so an SDK upgrade never outruns a server upgrade. `Config.DisableRequestCompression` opts out entirely.
...
Remote configuration is fetched from `GET {RemoteConfigURL}/config/v1/{workspace_id}/{environment_id}/{client_id}` (`Authorization: Bearer <APIKey>`, each path segment percent-escaped, `If-None-Match` revalidation when a cached ETag exists; no body, and never the schema-revision header — that is a batch-route contract). By default the request carries **no query parameters**; under the dark `RemoteConfigAttributesEnabled` opt-in — and only while consent is granted — the targeting attributes set via `SetRemoteConfigAttributes` ride as percent-escaped query parameters in the experiment attribute vocabulary (allowlist + `custom_attribute_<name>`, ≤512-byte values, 64-attribute cap, sorted; out-of-vocabulary keys dropped client-side, never sent).
...
The rejection ring and default diagnostics are unreleased. `client.Rejections()`
returns oldest-first copies of `BatchEventStatus` values: `EventID`, `Status`,
`Code`, and `Message`. `Config.RejectionCapacity` uses 64 for non-positive
values. Reads are safe during concurrent publishes and inside diagnostics
hooks. `Close` leaves this client's history available for inspection; a new
client starts with an empty ring.

`Track` and `Flush` keep returning a nil error for a parsed `202`, including
partial rejection. Inspect `Rejections()`, cumulative `Snapshot().Rejected`,
and `Snapshot().LastError`; the last diagnostic identifies a rejected event
without incrementing `FailedBatches`. Later accepted responses do not erase
that history. `BatchResult` already carries its rejected count and full
per-event entries to `OnBatchResult`; Go does not add a flush-result object.
...
optional and keeps its existing terminal-settlement behavior. Other per-event
statuses continue through `OnBatchResult` and `Snapshot().ByStatus`.

See the [ingest batch contract](https://docs.shardpilot.com/api/ingest/#batch-verdicts).
...
- **Server-side targeting attributes (dark, opt-in).** `RemoteConfigAttributesEnabled: true` (requires `RemoteConfigURL`) plus `SetRemoteConfigAttributes(map[string]string)` makes fetches carry targeting attributes as query parameters so server-side delivery rules can target this client; `nil`/empty clears the set, and the setter is inert while the opt-in is off. The vocabulary and bounds are the experiment consumer's, verbatim (`geo`, `app_version`, `device_type`, `install_date`, `user_segment`, plus `custom_attribute_<name>`; out-of-vocabulary names and unusable values are dropped client-side, never sent; ≤512-byte values, 64-attribute cap, sorted). **Attributes ride ONLY while consent is granted**: under unknown consent or either denied state (forced-minor included) the fetch still happens — config delivery stays consent-neutral — but attribute-less, serving whatever the server publishes for an untargeted client (typically the defaults). That granted-only gate is deliberately *stricter* than this SDK's open-under-unknown analytics posture: attributes describe the actor, so "unknown = zero bytes of personal data" holds on this leg even on the Go SDK. Targeting stays 100% server-evaluated (the SDK never evaluates rules). Last-known-good caching stays scope-keyed for value serving — a cached body may reflect the previously sent attribute set until the next successful fetch (documented v1 limit) — while the cached ETag revalidates only a fetch carrying the **same** attribute signature: any signature change (opting in, changing the set, a consent downgrade) forces a full fetch instead of risking a false 304 under a shared validator. The signature is an in-memory equality token only — nothing attribute-derived is ever persisted, and an attributed record's ETag is not persisted either (a reloaded record could not prove a same-signature fetch), so the first fetch after a restart of a previously-attributed record is a full fetch on every leg. Under the opt-in `ConsentFloor`, the grant-receipt dispatch gate holds this leg exactly like the event legs: while an analytics-grant receipt is retained undispatched, fetches stay attribute-less, so attributes can never overtake the grant on the wire on a strict-consent workspace.
...
`ExperimentsEnabled: true` opts this client into the experiment-assignment consumer. **The default is off — dark**: zero experiment code paths execute (no subject-id mint, no fetch, no revalidation goroutine, no fact emission, no new persistence keys), and the surface refuses `ErrExperimentsNotConfigured`. The flag requires `RemoteConfigURL` (the assignment endpoint lives on the same host, path-swapped to `/api/v1/runtime/experiments/assignment`) and authenticates with the same publishable `APIKey`.

- **Dark phase, stated plainly:** turning the flag on does not turn the feature on. Server-side enablement is separate and is not in this SDK's hands, so until it is done an enabled consumer receives `403` on every fetch — the expected fail-closed posture — and a publishable key without the `experiment_assignment_read` scope receives `401`. The exposure/outcome lane is dark **end-to-end** too: `experiment_exposure`/`experiment_outcome` are rejected per event for publishable client keys until the lane is enabled, and this SDK deliberately relies on that server-side block staying authoritative rather than gating locally.
- **Fetching:** `FetchExperimentAssignment(ctx, experimentKey, attributes)` issues one GET and returns the parsed verdict — assigned (variant key + payload + version + boundary), or one of the three not-assigned shapes distinguished by `Reason` (absent = deterministic traffic-gate miss, `kill_switch` = operator kill, `targeting_unmatched` = targeting miss; all three drop the cached assignment). `attributes` is the server-evaluated targeting vocabulary (`geo`, `app_version`, `device_type`, `install_date`, `user_segment`, plus `custom_attribute_<name>`); out-of-vocabulary names and unusable values are dropped, never sent. Targeting is 100% server-evaluated.
- **Failure taxonomy** (the remote-config canon, plus this plane's extras): transients (offline, `408`, `429`, `5xx` — the kill-state `503` included — malformed bodies) serve the durable last-known-good assignment with `FromCache=true`; `Retry-After` is honored on `429` and `5xx` and paces the *revalidation cadence only* (explicit fetches stay one GET, classified per fetch). `401`/`403` fail closed: serving stops and automatic revalidation halts until a later authorized fetch or re-init — the durable record is retained, EXCEPT for the server's real-subjects kill sentinel (`experiment real-subject assignment is disabled`), which drops the cached assignments and their subject-fact keys outright. `404` and non-grammar `400`s are permanent and drop the cached entry (never served stale again); the subject-grammar `400` sentinel re-mints the subject id once per process and retries with the same attribute set.
- **Subject id (`spcid_`):** SDK-minted and SDK-managed — `spcid_` + 32 lowercase hex (a UUIDv7, dashes stripped). No host override path exists: no setter, no config field. With `SpoolDir` it persists (`experiment_subject_key`, 0600) and sticks across restarts (any wire-valid stored id is kept verbatim — re-minting re-buckets); without `SpoolDir` it is per-process, documented ephemeral. It egresses ONLY as the assignment fetch's `subject_key` — never in analytics events or props.
- **Serving:** `ExperimentVariant(key)` / `ExperimentVariantPayload(key)` read the in-memory assignment (last-known-good across restarts with `SpoolDir`); they never touch the network and serve zero values while nothing is cached, while `401`/`403` latched, or while consent refuses.
- **Consent (granted-only plane):** the consumer consumes the SAME effective consent state the analytics path uses — denial (either flavor, forced-minor included) closes the whole plane (no fetches, no revalidation, no serving, no facts; a forced-minor session produces ZERO experiment traffic). Under the opt-in `ConsentFloor`, unknown consent closes it too; floor-off keeps this SDK's documented open-under-unknown posture on this plane as well. A consent downgrade retains the durable cache unserved; a re-grant serves it again. This is deliberately stricter than the remote-config fetch (not consent-gated).
- **Revalidation (the kill-switch reach):** while assignments are cached, a background lane re-fetches each one every 300s ± 10% jitter, so a running client converges on an operator kill or a republish within one interval — an offline client keeps its variant indefinitely (the documented kill-latency caveat). Transient failures back off (transport parity), a `Retry-After` parks the cadence (clamped to one day), and the lane never blocks `Close`.
- **Exposure/outcome facts:** ride the normal analytics pipeline (queue → batch → spool → consent gates) with the strict server-side props allowlist (`experiment_key`, `experiment_version`, `assignment_key`, `variant_key`, `assignment_unit`, plus `outcome_key`/`outcome_value` on outcomes). `assignment_key` carries the server-minted `sfk1_` subject-fact key VERBATIM — an assignment without one emits no fact. Envelope: `source` is always `client`, `user_id` is always omitted, `anonymous_id` carries the configured client identity (erasure reachability; facts without a configured `AnonymousID` are skipped). One exposure emits automatically per (experiment, version, subject) per client instance at the assignment's application, with a DETERMINISTIC event id so retries and re-emissions collapse server-side; `TrackExperimentExposure(key)` is the explicit re-arm (an extra fact, distinct id); `TrackExperimentOutcome(key, outcomeKey, value)` emits host-triggered outcome facts. Owed emissions (full queue, consent-closed windows, cache restores) stay armed and drain on the lane's sweep — and once more at `Close`, after the flush frees room.
...
- **Consent-regime plans (`pkg/consentpolicy`) — what this release's plan-text validation covers, and what it does not.** `Prepare` verifies a plan for one scoped player operation and, **in this release, uses no plan at all**: there is no key to authenticate one with, an unauthenticated plan is not evidence, so every verdict is closed on every axis (`PlanUsed()` false, every lane shut, every purpose reported prohibited and every operation blocked). Validation still runs first, so a malformed or out-of-scope handoff is named precisely, and it covers **raw UTF-8 validity of the plan bytes, exact-case unique keys at every depth, presence of every required scalar (absence is never a default), and EOF after the plan**. Two things it deliberately does **not** cover, because `encoding/json` and `time.Parse` repair rather than refuse them: **JSON escape-level unpaired surrogates**, which decode to U+FFFD, and **out-of-range RFC 3339 zone offsets**, which normalise. Neither can change a decision while no plan is used; both are **required of the release-2 verifier**, the one that has a verification key, and are recorded here so that release does not have to rediscover them.
