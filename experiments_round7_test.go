...
func TestPostPurgeFreshExposureSurvivesFilter(t *testing.T) {
	...
	stale, skip := client.buildExperimentFactEvent(experimentExposureName, "exp-stale", entry, "")
	if skip != "" {
		t.Fatalf("test setup: fact build refused (%s)", skip)
	}
	client.expFactPurgeEpoch.Add(1) // the sentinel lands
	fresh, skip := client.buildExperimentFactEvent(experimentExposureName, "exp-fresh", entry, "")
	if skip != "" {
		t.Fatalf("test setup: fact build refused (%s)", skip)
	}
	backoff := 0
