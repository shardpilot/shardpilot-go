...
# topmost released (non-Unreleased) section of CHANGELOG.md.
#
# The Go SDK has no in-code version constant — Go modules version via git
# tags — so the consistency surface is README ↔ CHANGELOG. This runs on every
# CI build and must not require git tags to be present (CI checkouts are
# shallow and tagless).
#
# Usage:
#   scripts/check_release_consistency.sh             # README ↔ CHANGELOG only
